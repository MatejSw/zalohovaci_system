...
namespace zalohovaci_system.Services
{
    public class JobINCR : Job
    {
        public override void Backup(BackupJob job, List<Snapshot> snapshots)
        {
            ...
            DateTime snapshot = snapshots.OrderByDescending(s => s.CreatedAt).FirstOrDefault()?.CreatedAt ?? DateTime.MinValue;

            foreach (string source in job.Sources)
            {
                foreach (string file in Directory.GetFiles(source, "*", SearchOption.AllDirectories))
                {
                    if (File.GetLastWriteTime(file) <= snapshot && job.Method != BackupMethod.Full && Directory.GetFiles(source, "*", SearchOption.AllDirectories).Length != 0 && !fullBackup) continue; // Soubor nebyl změněn od posledního snapshotu
            ...
            foreach (string target in job.Targets)
            {
                ...
                else
                {
                    ...
                    ZipFile.CreateFromDirectory(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Swientek_Backuper", "temp"), Path.Combine(target, "current_package", $"incr_{x}.zip"));
                    ZipFile.ExtractToDirectory(Path.Combine(target, "current_package", $"incr_{x}.zip"), Path.Combine(target, "current_package", $"incr_{x}"));
                    File.Delete(Path.Combine(target, "current_package", $"incr_{x}.zip"));
                }
            }
